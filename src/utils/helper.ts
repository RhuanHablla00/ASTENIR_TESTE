...
export function updateCache<T>(
  ...
  endpoint: string,
  arg: any,
  update: UpdateValue<T> | Updater<T>
) {
  const isUpdater = typeof update === "function";

  dispatch(
    api.util.updateQueryData(endpoint, arg, (draft: T) => {
      if (isUpdater) {
        (update as Updater<T>)(draft);
      } else {
        return update as T;
      }
    })
