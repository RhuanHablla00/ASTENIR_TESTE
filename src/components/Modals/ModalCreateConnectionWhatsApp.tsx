...
import { useAuthenticateMetaApiMutation, UseCaseType, useCreateFanpageAndInstagramConnectionApiMutation, useCreateWhatsAppConnectionApiMutation, useGetAllConnectionsQuery } from "@/api/connectionsApi";
import { useAppSelector } from "@/stores/hooks";
import { useParams } from "react-router-dom";
import { useTranslation } from "react-i18next";
import { showErrorNotification, showSuccessNotification } from "../Base/Notification";
import { useWhatsappMesssagingQuery } from "@/api/authApi";
...
export default function ModalCreateConnectionWhatsapp({
  onSuccess,
}: {
  onSuccess: () => void;
}) {
  ...
  const [createWhatsappConnection, { isLoading }] = useCreateWhatsAppConnectionApiMutation();

  const { data: whatsappConnectionData } = useWhatsappMesssagingQuery({ use_global: true });

  console.log("whatsappConnectionData", whatsappConnectionData);
  ...
  const togglePhoneSelection = (pageId: string) => {
  ...
  const handleFormSubmit = async (formData: any) => {
    try {
      ...
      const formattedPhones = finalData.phones
        .filter((phone: any) =>
          finalData.selectedPhones.includes(
            phone.whatsapp_phone_number_id
          )
        )
        .map((phone: any) => ({
          whatsapp_phone_number_id: phone.whatsapp_phone_number_id,
          whatsapp_business_account_id:
            phone.whatsapp_bussiness_account_id,
          name: finalData.connectionName,
        }));
      ...
      await createWhatsappConnection({ body }).unwrap();
      ...
      <div className="col-span-12">
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="mt-7">
          <div className="pb-7">
              ...
              <div className="flex-col block pt-5 mt-5 xl:items-center sm:flex xl:flex-row first:mt-0 first:pt-0">
                ...
                <div className="flex-1 w-full mt-3 xl:mt-0">
                  <FormInput
                    ...
                    placeholder={`Ex: ${t("my_whatsapp_connection")}`}
                  />
                    ...
                    onClick={() => {
                      window.FB.login((response: any) => {
                        ...
                        if (response.authResponse && response.status === "connected") {
                          ...
                          if (code && workspace && connection_type) {
                            setCurrentStep(3);
                            setPhonesLoading(true);

                            authenticateMeta({ code, scope: connection_type.toUpperCase() })
                              .unwrap()
                              .then((result) => {
                              })
                              .catch((err) => {
            ...
            {currentStep === 3 && (
              <div className="animate-[fadeIn_0.4s_ease-in-out] space-y-6">
                ...
                <div className="mb-8">
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                    {t('select_a_page_to_finish_creating_the_connection')}
                  </p>
                </div>
                {phonesLoading ? (
                  ...
                  <div className="grid gap-4 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3 mb-4">
                    {data.phones.map((phone: any) => {
                      const isSelected = selectedPhones.includes(
                        phone.whatsapp_phone_number_id
                      );

                      return (
                        <div
                          key={phone.whatsapp_phone_number_id}
                          className={clsx(
                            "p-4 border rounded-md shadow-sm bg-white dark:bg-darkmode-800 cursor-pointer relative transition-all duration-200",
                            {
                              ...
                              "border-slate-200 dark:border-darkmode-600": !isSelected,
                            }
                          )}
                          onClick={() =>
                            togglePhoneSelection(phone.whatsapp_phone_number_id)
                          }
                        >
                          {isSelected && (
                          ...
                          <div className="flex items-center gap-3 mb-3">
                            {phone.whatsapp_profile_picture_url ? (
                              <img
                                src={phone.whatsapp_profile_picture_url}
                                alt={phone.whatsapp_phone_number}
                                className="w-12 h-12 rounded-full object-cover"
                              />
                            ) : (
                              <div className="w-12 h-12 rounded-full bg-slate-200 dark:bg-darkmode-600 flex items-center justify-center text-slate-500 text-xs">
                                WA
                              </div>
                            )}
                          </div>
                          ...
                          <div className="text-xs text-slate-500 dark:text-slate-400 mb-1">
                            Account ID: {phone.whatsapp_bussiness_account_id}
                          </div>

                          {phone.whatsapp_business_account_currency && (
                            <div className="text-xs text-slate-500 dark:text-slate-400">
                              Currency: {phone.whatsapp_business_account_currency}
                            </div>
                          )}
                        </div>
                      );
          ...
          <div className="flex w-full justify-end gap-4">
            ...
            {currentStep < totalSteps ? (
              ...
              <Button
                ...
                variant="primary"
                className="px-6"
                disabled={isLoading || selectedPhones.length === 0}
              >
                {isLoading ? (
                  <>
                    <Lucide icon="Loader2" className="w-4 h-4 mr-2 animate-spin" />
                    {t("creating_connection")}
                  </>
                ) : (
                  <>
